...
export class SourceIds implements IdGenerator {
  key = ''
  generateTraceId(): string { return createHash('sha256').update(`gh:v3:trace:${this.key}`).digest('hex').slice(0, 32) }
  generateSpanId(): string { return createHash('sha256').update(`gh:v3:span:${this.key}`).digest('hex').slice(0, 16) }
}
