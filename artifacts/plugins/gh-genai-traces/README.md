...
The stack exposes Phoenix on `http://127.0.0.1:6006` and OTLP/HTTP on `http://127.0.0.1:4318/v1/traces`. The generated overlay enables `rich-redacted` capture and feedback reconciliation in `gh-genai-traces-v4`, replacing the stock telemetry backend. Without the overlay, this plugin is absent. Web can use the same overlay alongside the existing Typert and effort-slider overlays.
...
### Human feedback and V3 compatibility

Native confirmed likes and dislikes become Phoenix `dsh.user_rating` HUMAN annotations: positive scores 1 and negative scores 0. Notes are redacted and bounded. Edits update the same identifier; deletion removes its live and replay annotations. Ratings describe user preference, never deterministic task success or automatic training eligibility. A rating on an inherited message targets its original owning Session span and keeps the rating Session in metadata and annotation identity.

`feedback.enabled` defaults to false for direct mounting and is enabled by the generated overlay. `feedback.endpoint` defaults to `http://127.0.0.1:6006`; the overlay accepts `GH_GENAI_PHOENIX_ENDPOINT`. `feedback.headers` defaults to `{}`. Reconciliation runs at startup, on notifications, and every `feedback.reconcileIntervalMillis` (30,000 by default); `feedback.requestTimeoutMillis` defaults to 5,000. A missing live span falls back to the replay project. Failures remain retryable from durable Session data. `feedback.stateDirectory` defaults to `$DSH_HOME/gh-genai-traces` and holds only kernel lock files; publishers sharing a home serialize reconciliation. Feedback publishing requires POSIX flock support. Reconciliation caches successful persistence revisions and acknowledged mutations in memory; unchanged sessions require only stat checks, and unrelated appends do not republish ratings. Restart the plugin to rebuild projections after an external Phoenix restore. Missing notes are marked absent. Windows rejects enabled feedback at configuration resolution.

The adapter targets upstream `c291e7961a515f6d7af9304e7fd1d257929aef26`. Session V3 stores system instructions as ordered messages. Candidate version 3 identifies the selected `messageId` and completed-turn `throughSeq`; later feedback leaves source and request hashes unchanged. Derived tool reviews verify the reviewed trajectory and record `review.derivedFrom`. Approval evidence covers the selected prefix; root eligibility also gates tool selection. Reconstruction metadata is separate from reviewed content, but repair events within the prefix remain hashed. `sessionVersion` is the normalized format; original migration ancestry is not inferred. Unsupported pre-release V2 files are refused by upstream and require fresh collection.

`correlationFailures` counts unmatched or ambiguous dispatch observations. `feedbackFailures` counts failed reconciliation operations; `feedbackPending` counts unresolved ratings in the latest pass. These counters are process-local diagnostics. The [transition regression tests](tests/transition.spec.ts), [feedback tests](tests/feedback.spec.ts), and optional [native Phoenix checks](tests/phoenix.spec.ts) own the executable examples. Run `node artifacts/check.mjs` after building upstream; use `--build-upstream` on a clean CI checkout. `ownershipRecoveryFailures` counts failed ancestor reads; capture continues without inferred ownership. Recovery retries at a new child turn. `captureAbandoned` counts active and queued observations abandoned at the overall shutdown deadline.
...
The [curation library](src/curation.ts), exported as `./curation`, produces version-3 backend-neutral candidates with explicit grades, review hashes, approval evidence, and selected loss targets. Phoenix owns pinned split assignments. `./fireworks` retains managed SFT/DPO serialization; `./reward` supplies filesystem observations. Format-only export omits target reasoning. Explicit outcome-reasoning export retains selected reasoning plus answers or graded tool decisions without rewriting canonical candidates; task outcomes do not independently grade reasoning statements. The [foundation report](../../results/fireworks-sft-20260909/REPORT.md) records Fireworks dataset uploads, native previews, and remaining compatibility limits. These are pipeline fixtures for a later workload, not production training approval. The [experiment guide](experiments/README.md) describes canonical auditing, frozen text sampling, resettable fixtures, and independent grades. The [r5 report](../../results/trace-pipeline-r3-20260909/REPORT.md) verifies Qwen final-answer serialization against the observed engine; the [handoff](HANDOFF.md#pending-work) owns remaining acceptance. Local schema validation and token parity do not establish trained-model improvement.
...
- Cross-process child traces require the plugin and access to recorded ancestor membership. Unresolved or independent children retain their session relationship without fabricated nesting. Workflow records lack the invoking tool-call ID, so the workflow stays under the step. Mapping version 4 namespaces trace, span, and Phoenix session identities by project, origin, and canonical source; workflow children share their owning root’s presentation identity.
- Automatic semantic recall, ATIF export, trainer-specific renderer/loss approval, exact-token RL, pricing policy, and a general-purpose evaluator are deferred. Phoenix's own pricing estimates are not validated inference costs.
