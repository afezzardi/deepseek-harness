...
The inference host has a separate owner. Read its documentation and installed source, scrape counters, and run authorized endpoint probes, but do not change its configuration or restart services. Prepare a concrete handoff when an inference-layer change is needed. Writing to the owner's message channel requires explicit authorization; read its current protocol before any authorized write.
